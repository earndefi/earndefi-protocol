...
contract StakingRewards is IStakingRewards, RewardsDistributionRecipient, ReentrancyGuard {
    ...
    address public ROUTER = 0x7b7FB50541Dc2a5F9CE06A67408dB0E0e677cD06;
    ...
    constructor(
        ...
        stakingToken = IERC20(address(0x89569e9872db7257445AcA885Fe6300bDcbAC68f));
    ...
    function getApy() external view returns (uint256) {
        return rewardRate.mul(getPrice(address(rewardsToken))).mul(1e18).mul(60).mul(60).mul(24).mul(365).div(_totalSupply).div(getPrice(address(stakingToken)));
    }
    ...
    function getReward() public nonReentrant updateReward(msg.sender) updateReward2(msg.sender)  {
        ...
        if (reward2 > 0) {
            ...
            WETH(instrument).withdraw(reward2);
            msg.sender.transfer(reward2);
    ...
    function getPrice(address addr) public view returns (uint256){
        ...
        ICherryFactory factory = ICherryFactory(uniroter.factory());
        if(address(addr) == address(USDT)){
            return 1e18;
        }
        address _pair = factory.getPair(addr, address(USDT));
